...
handleWebSocketMessage: (state, action: PayloadAction<any>) => {
  ...
  if (state.currentChat && message.chatId === state.currentChat.chat_id) {
    ...
    state.chatList = {
      ...state.chatList,
      data: state.chatList.data.map((chat: { chat_id: any, unread_count?: number }) =>
...
export const { setCurrentChat, setChatStory, addMessageToChatStory, handleWebSocketMessage  } = chatSlice.actions
